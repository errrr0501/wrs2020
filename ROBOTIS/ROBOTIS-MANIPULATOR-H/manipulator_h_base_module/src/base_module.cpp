                         ...
                         std::map<std::string, double> sensors)
{
  ...
  if(robotis_->is_moving_ == true && timer1 >= 4)
  {
    ...
    if(std::abs(bias_cur[1]) >= (avg_cur1+4) && init_var != true)  //bias_pos[1] >= 300 && bias_cur[1] >= 100 (joint_speed)*10
    {
      data_latch1 = true;
      if_hit[1]++;
      if(detect_hit[1] == 0)
    ...
    if(std::abs(bias_cur[2]) >= (avg_cur2+4) && init_var != true)  //bias_pos[2] >= 100 && bias_cur[2] >= 85  //(joint_speed)*5+150) (joint_speed)*5+50
    {
      data_latch2 = true;
      if_hit[2]++;
      if(detect_hit[2] == 0)
...
//    ros::Time time = ros::Time::now();
  ...
  {
    // std::cout<<"====robotis_->is_moving_ == true====="<<std::endl;
    ...
    {
      // std::cout<<"====ik_solve_ == true====="<<std::endl;
